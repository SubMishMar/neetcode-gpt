...
class Solution:
    def generate(self, model, new_chars: int, context: TensorType[int], context_length: int, int_to_char: dict) -> str:
        ...
        for i in range(new_chars):
            ...
            logits = model(cropped_context) # BxTxvocab_size
            B, T, vocab_size = logits.shape
            last_logits = logits[:,-1,:] # Bxvocab_size
            probabilities = nn.functional.softmax(last_logits, dim=1)
            generator.set_state(initial_state)
            # MORE OF YOUR CODE (arbitrary number of lines)
            next_token = torch.multinomial(probabilities, 1, generator=generator) # Bx1
