...
class Solution:
    def get_positional_encoding(self, seq_len: int, d_model: int) -> NDArray[np.float64]:
        ...
        dimension_ids = np.arange(d_model).reshape(1, -1)
        div_terms = 10000 ** (np.arange(0, d_model, 2) / d_model)
        PE[:, 0::2] = np.sin(position_ids / div_terms)
        PE[:, 1::2] = np.cos(position_ids / div_terms)
