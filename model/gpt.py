...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.position_embdedding = nn.Embedding(context_length, model_dim)
        self.transformer_block = nn.Sequential(*[self.TransformerBlock(model_dim, num_heads) for _ in range(num_blocks)])
        self.layer_norm = nn.LayerNorm(model_dim)
        self.linear = nn.Linear(model_dim, vocab_size, bias=True)
